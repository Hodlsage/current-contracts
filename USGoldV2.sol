...
library Counters {
    ...
    function currentv2(Counter storage counter) internal view returns (uint256) {
...
contract USGv2 is ERC721Enumerable, Ownable {
    using Strings for uint256;
    using SafeMath for uint256;
    using Counters for Counters.Counter;
 
    string public baseExtension = ".json";
    uint256 public cost = 2770 ether;
    uint256 public startPrice = 2770 ether;
    address public currentv2 = 0x7Ce8E3780F6C688C11039917f40563ECFDCCd0d8;
 
    uint256 public mintReward = 1;
    ...
    uint256[10000000] public remainingIds;
    ...
    function mint(uint256 _mintAmount) public {
        require(!paused, "USGv2: minting has not started yet");
        require(_mintAmount > 0, "USGv2: you have to mint at least one");
        require(_mintAmount <= maxMintAmount,  "USGv2: you can only mint 10 at a time");
        require(lastSupply >= _mintAmount,  "USGv2: the collection is sold out");
            if (msg.sender != owner()) {
                IERC20(currentv2).transferFrom(msg.sender, address(this), cost * _mintAmount);
            }
    ...
    function randomGiveaway(address _winner, uint256 _amount) external onlyOwner {
        require(_winner != address(0), "USGv2: zero address");
        require(_amount > 0, "USGv2: you have to mint at least one");
        require(lastSupply != 0,  "USGv2: the collection is sold out");
    ...
    function tokenURI(uint256 _id) public view virtual override returns (string memory){
        require(_exists(_id),"USGv2: URI query for nonexistent token");
        string memory currentv2BaseURI = _baseURI();
        return bytes(currentv2BaseURI).length > 0
        ? string(abi.encodePacked(currentv2BaseURI, _id.toString(), baseExtension))
        : "";
    }
    ...
    function claimRewards() public {
        ...
        IERC20(currentv2).transfer(msg.sender, balance);
    }
